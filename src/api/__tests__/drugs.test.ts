...
describe('Drug API', () => {
  ...
  describe('addDrug', () => {
    ...
    it('should handle duplicate drug errors', async () => {
      ...
      await expect(addDrug(duplicateDrug)).rejects.toThrow('Drug already exists')
    })
  ...
  describe('updateDrug', () => {
    ...
    it('should handle drug not found error', async () => {
      ...
      await expect(updateDrug(drugId, updates)).rejects.toThrow('Drug not found')
      expect(api).toHaveBeenCalledWith(`/drugs/${drugId}`, {
        method: 'PUT',
        body: JSON.stringify(updates)
      })
    ...
    it('should handle invalid update data', async () => {
      ...
      await expect(updateDrug(drugId, invalidUpdates)).rejects.toThrow('Invalid update data')
    })
  })
  ...
  describe('deleteDrug', () => {
    ...
    it('should handle drug in use error', async () => {
      ...
      await expect(deleteDrug(drugId)).rejects.toThrow('Cannot delete drug: drug is in use')
    })
  ...
  describe('Error Scenarios', () => {
    it('should handle malformed JSON response', async () => {
      ...
      await expect(getDrugs()).rejects.toThrow('Unexpected token < in JSON at position 0')
    })
