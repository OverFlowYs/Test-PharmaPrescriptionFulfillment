...
vi.mock('element-plus', () => ({
  ...
  ElInput: {
    name: 'ElInput',
    template: '<input v-bind="$attrs" v-model="modelValue" @input="$emit(\'input\', $event.target.value)" />',
    props: ['modelValue', 'placeholder', 'type', 'maxlength'],
    emits: ['input', 'update:modelValue']
  },
  ElInputNumber: {
    name: 'ElInputNumber',
    template: '<input type="number" v-bind="$attrs" v-model="modelValue" @input="$emit(\'input\', $event.target.value)" />',
    props: ['modelValue', 'min'],
    emits: ['input', 'update:modelValue']
  },
  ElDatePicker: {
    name: 'ElDatePicker',
    template: '<input type="date" v-bind="$attrs" v-model="modelValue" @input="$emit(\'input\', $event.target.value)" />',
