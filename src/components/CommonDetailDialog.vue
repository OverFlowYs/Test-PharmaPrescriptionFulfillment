<template>
  ...
  >
    <div v-if="data">
      ...
      <el-descriptions :column="2" border>
        ...
        >
          <template v-if="field.slot">
            <slot :name="field.slot" :value="getValue(data, field.key)" :data="data">
          ...
          <el-table :data="getValue(data, subTable.dataKey)" size="small">
            ...
            >
              <template #default="{ row }" v-if="column.slot">
                <slot :name="`${subTable.key}-${column.slot}`" :row="row" :column="column">
      ...
      <!-- 错误信息 -->
      <div v-if="getValue(data, 'failureReasons') && getValue(data, 'failureReasons').length > 0" class="mt-4">
        <h4>失败原因</h4>
        <el-alert
